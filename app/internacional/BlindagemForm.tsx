...
import { submitBlindagemLead, type BlindagemFormState } from "@/lib/actions";
import { pushEvent } from "@/lib/analytics";

/**
 * Formulário de qualificação da LP /blindagem (proteção internacional).
 *
 * Multi-step: motivação → faixa → horizonte → dados. Micro-compromissos
 * convertem melhor que um form longo num produto de ticket relevante.
 * As escolhas 1–3 viajam como hidden inputs; só o passo 4 tem submit.
 * Conversão medida SOMENTE em sucesso real (evento lead_form_submit).
 *
 * A faixa roteia a velocidade do atendimento (score A/B/C no servidor) —
 * não é barreira: ninguém é excluído por patrimônio.
 */

const initial: BlindagemFormState = { kind: "idle" };
const emptyVals = {
  name: "",
  whatsapp: "",
  email: "",
...
const STEPS: {
...
}[] = [
  {
    key: "motivacao",
    label: "Motivação",
    question: "O que mais pesa na sua decisão de proteger patrimônio fora do Brasil?",
    options: BLINDAGEM_MOTIVACOES,
  },
  {
    key: "faixa",
    label: "Patrimônio",
    question: "Quanto você buscaria proteger no exterior?",
    options: BLINDAGEM_FAIXAS,
  },
  {
    key: "horizonte",
    label: "Horizonte",
    question: "Em que prazo você pensa essa proteção?",
    options: BLINDAGEM_HORIZONTES,
  },
];

export function BlindagemForm() {
  const [state, action, pending] = useActionState(submitBlindagemLead, initial);
  ...
  useEffect(() => {
    if (state.kind === "success") {
      pushEvent("lead_form_submit", { form_page: "internacional" });
    }
  }, [state.kind]);

  function choose(key: "motivacao" | "faixa" | "horizonte", value: string) {
    setChoices((c) => ({ ...c, [key]: value }));
    const next = Math.min(step + 1, TOTAL_STEPS - 1);
    setStep(next);
    // Evento por etapa: mede o funil interno do form no GTM/GA4.
    pushEvent("internacional_form_step", { form_page: "internacional", step: step + 1, field: key });
    ...
    return (
      <div role="status" aria-live="polite" className="rounded-2xl bg-white border border-[#EDEFF2] p-8 md:p-10 shadow-sm">
        ...
        <p className="text-[1rem] leading-[1.65]" style={{ color: "#6B7B8D" }}>
          A conversa é reservada e sem compromisso. Vamos entender o seu caso e mostrar,
          em detalhe, como estruturar a sua proteção internacional em dólar.
        </p>
      </div>
    );
  }
    ...
    <div ref={topRef} className="rounded-2xl bg-white border border-[#EDEFF2] p-6 md:p-8 shadow-sm">
      ...
      {!isDataStep && (
        <div>
          ...
          <p className="text-[0.78rem] leading-relaxed mt-5" style={{ color: "#9BA8B5" }}>
            Conversa reservada. Não solicitamos dados bancários, senhas ou extratos. Leva cerca de dois minutos.
          </p>
        </div>
      )}
      ...
      {isDataStep && (
        <form action={action} noValidate className="flex flex-col gap-5">
          ...
          <input type="hidden" name="motivacao" value={choices.motivacao} />
          <input type="hidden" name="faixa" value={choices.faixa} />
          <input type="hidden" name="horizonte" value={choices.horizonte} />
          ...
          <div>
            <label htmlFor="bl-name" className={labelClass} style={{ color: "#4a6b8c" }}>
              Nome completo
            </label>
            <input
              id="bl-name"
          ...
          <div>
            <label htmlFor="bl-whatsapp" className={labelClass} style={{ color: "#4a6b8c" }}>
              WhatsApp
            </label>
            <input
              id="bl-whatsapp"
          ...
          <div>
            <label htmlFor="bl-email" className={labelClass} style={{ color: "#4a6b8c" }}>
              E-mail
            </label>
            <input
              id="bl-email"
          ...
          <label className="flex items-start gap-3 text-[0.8rem] leading-[1.5] mt-1" style={{ color: "#6B7B8D" }}>
            ...
            <span>{BLINDAGEM_CONSENT_TEXT}</span>
          </label>
